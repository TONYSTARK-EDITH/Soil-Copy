...
% SOIL_DETECT MATLAB code for Soil_Detect.fig
%      SOIL_DETECT, by itself, creates a new SOIL_DETECT or raises the existing
%      singleton*.
%
%      H = SOIL_DETECT returns the handle to a new SOIL_DETECT or the handle to
%      the existing singleton*.
%
%      SOIL_DETECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SOIL_DETECT.M with the given input arguments.
%
%      SOIL_DETECT('Property','Value',...) creates a new SOIL_DETECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Soil_Detect_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Soil_Detect_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Soil_Detect

% Last Modified by GUIDE v2.5 30-Mar-2022 10:05:10
...
% --- Executes just before Soil_Detect is made visible.
function Soil_Detect_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Soil_Detect (see VARARGIN)

% Choose default command line output for Soil_Detect
...
% UIWAIT makes Soil_Detect wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Soil_Detect_OutputFcn(~, ~, handles) 
...
sen=Sensitivity_image(label2rgb(L, @jet, [.5 .5 .5]));
spe=specificity_image(label2rgb(L, @jet, [.5 .5 .5])); 
set(handles.edit9,'String',acc);
set(handles.edit10,'String',sen);
set(handles.edit11,'String',spe);
 run('Soil_Available_Analysis.p'); 
 run('Soil_Predict_Analysis.p');
